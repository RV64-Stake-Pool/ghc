...
import Control.Concurrent ( forkIO, QSem, newQSem, waitQSem, signalQSem, ThreadId, myThreadId)
...
createBuildPlan mod_graph maybe_top_mod =
    let -- Step 1: Compute SCCs without .hi-boot files, to find the cycles
        ...
        build_plan = toBuildPlan cycle_mod_graph []
...
load' how_much mHscMessage mod_graph = do
    ...
    (upsweep_ok, hsc_env1) <- withDeferredDiagnostics $
      liftIO $ upsweep_fn hsc_env mHscMessage pruned_hpt transitive_deps build_plan
    setSession hsc_env1
    case upsweep_ok of
...
* The amount of parrelism is controlled by a QSem, runLoop uses the QSem to only
  keep N live threads at once. This prevents spawning a large number of threads, which
  are all blocked on a lot of MVars. It can be slow if there are a lot of waiting
  threads which wake-up to make a tiny bit of progress before blocking again.
...
* The result variable for an action producing `a` is of type `Maybe a`, therefore
  it is still filled on a failure. Therefore if a module fails to compile, the
...
interpretBuildPlan deps_map plan = do
  ...
  where
    ...
    buildSingleModule knot_var mod = do
      ...
      return $ (MakeAction default_action res_var, result_var)
...
parUpsweep n_jobs hsc_env _mHscMessage old_hpt transitive_deps build_plan = do
    (cycle, pipelines, collect_result) <- interpretBuildPlan transitive_deps build_plan
...
wrapAction hsc_env k = do
              ...
              -- directly. MP should probably use safeTry here to not catch async exceptions.
...
runPipelines n_jobs orig_hsc_env old_hpt all_pipelines = do
  ...
  -- The semaphore which limits the number of parralel jobs.
  ...
  let resetNumCapabilities orig_n = do
      ...
      env = MakeEnv { hsc_env = thread_safe_hsc_env
                    , old_hpt = old_hpt
                    , lqq_var = log_queue_queue_var
                    }

  MC.bracket updNumCapabilities resetNumCapabilities $ \_ ->
    runAllPipelines compile_sem env all_pipelines
...
runAllPipelines :: QSem -> MakeEnv -> [MakeAction] -> IO ()
runAllPipelines compile_sem env acts = do
  active_var <- newMVar Set.empty
  runLoop compile_sem active_var env acts
  mapM_ waitMakeAction acts

-- | Execute each action in order, limiting the amount of parrelism by the given
-- semaphore.
runLoop :: QSem -> MVar (Set.Set ThreadId) -> MakeEnv -> [MakeAction] -> IO ()
runLoop _compile_sem _ _env [] = return ()
runLoop compile_sem active_threads_var env (MakeAction act res_var :acts) = do
  -- Wait for a slot
  waitQSem compile_sem
  modifyMVar_ active_threads_var $ \active -> do
      -- When a slot is ready, start a pipeline
        new_thread <-
          forkIO $ (do
            mres <- (run_pipeline (withLocalTmpFS act))
                      `MC.finally` (signalQSem compile_sem >> -- Release back the semaphore slot
                                   (myThreadId >>= \tid -> modifyMVar_ active_threads_var (return . (Set.delete tid)))) -- Delete from active threads
        ...
        return (Set.insert new_thread active)
  runLoop compile_sem active_threads_var env acts
  where
